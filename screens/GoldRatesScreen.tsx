...
// ── PIN Pad ───────────────────────────────────────────────────────────────────
function PinPad({ onSuccess, onClose, shake }: { onSuccess: () => void; onClose: () => void; shake: Animated.Value }) {
...
function RateCard({ k, purity, price, featured, color, change }: {
  ...
  useEffect(() => {
    // Pulse scale on price change
    Animated.sequence([
      Animated.timing(scale, { toValue: 1.03, duration: 180, useNativeDriver: true }),
  ...
  const flashBg = flashClr.interpolate({
    inputRange:  [0, 1],
    outputRange: ['rgba(0,0,0,0)', change >= 0 ? 'rgba(22,163,74,0.08)' : 'rgba(220,38,38,0.08)'],
  });
...
export default function GoldRatesScreen() {
  ...
  // ── LIVE FLUCTUATION — ±1 or ±2 every 3 seconds ──────────────────────────
  useEffect(() => {
    if (rate24k === 0) return;
    const interval = setInterval(() => {
      ...
      KARATS.forEach(k => {
        ...
        newFluc[k.k] = sign * amount;
      });
      setFluctuation(newFluc);
    }, 3000);
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        {/* Hidden admin tap area — no visible text */}
              ...
              <>
                <View style={styles.adminHeader}>
                  <View>
                    ...
                    <Text style={styles.adminSub}>Saves locally on this device</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  tapHint:        { position:'absolute', top:-6, right:-6, backgroundColor:RED, borderRadius:8, width:16, height:16, alignItems:'center', justifyContent:'center' },
    ...
    <View style={pinStyles.dotsRow}>
      {Array.from({ length }).map((_, i) => (
        <View
          ...
          style={[
            pinStyles.dot,
            i < filled ? pinStyles.dotFilled : pinStyles.dotEmpty,
          ]}
        />
      ))}
    </View>
...
function RateCard({ k, purity, price, featured, color }: {
  ...
  return (
    <Animated.View style={[styles.rateCard, featured && styles.rateCardFeatured, { transform: [{ scale }] }]}>
      {featured && (
        <View style={styles.popularTag}>
          <Text style={styles.popularText}>MOST POPULAR</Text>
        </View>
...
export default function GoldRatesScreen() {
  ...
  // ── FIX 1: Load rates from website first, AsyncStorage as fallback ──────────
  useEffect(() => {
    const loadRates = async () => {
      try {
        // Try website first (live rates for all users)
        const res = await fetch(RATES_URL, { headers: { 'Cache-Control': 'no-cache' } });
        if (res.ok) {
          const data = await res.json();
          if (data.rate24k) {
      ...
      // Fallback: AsyncStorage (offline / website unreachable)
      try {
        const saved24k    = await AsyncStorage.getItem(STORAGE_KEY);
        const savedDate   = await AsyncStorage.getItem(DATE_KEY);
        const savedSilver = await AsyncStorage.getItem(SILVER_KEY);
  ...
  // ── Secret unlock (tap logo 5 times) ─────────────────────────────────────
  const handleLogoTap = () => {
    const next = tapCount + 1;
    setTapCount(next);
    clearTimeout(tapTimer.current);
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {rate24k > 0 ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyText}>
              Fetching live gold rates from our server. Please check your internet connection.
            </Text>
          </View>
        )}
        ...
        {/* FIX 2: "Tap here to update rates" text is now HIDDEN — invisible TouchableOpacity only */}
        <TouchableOpacity onPress={handleLogoTap} style={styles.adminHint} activeOpacity={1}>
          {/* Text removed — logo tap still works secretly */}
        </TouchableOpacity>
        ...
        >
          ...
          <Animated.View
            style={[styles.adminSheet, {
              transform: [{
                translateY: adminAnim.interpolate({ inputRange: [0, 1], outputRange: [600, 0] }),
              }],
            }]}
          >
            <View style={styles.sheetHandle} />
            ...
            {adminStep === 'form' && (
              <>
                ...
                <ScrollView
                  style={{ flex: 1 }}
                  contentContainerStyle={styles.adminBody}
                  keyboardShouldPersistTaps="handled"
                >
                  <Text style={styles.fieldLabel}>24K GOLD RATE (₹ per gram) *</Text>
                  <View style={styles.fieldRow}>
                  ...
                  {/* FIX 3: Updated admin note */}
                  <Text style={styles.adminNote}>
                    🌐 Live rates are fetched from nxtgenailabs.work/gold-rates.json — edit that file on GitHub to update all users instantly.
                  </Text>
                </ScrollView>
              </>
            )}
...
const styles = StyleSheet.create({
  ...
  disclaimer:    { fontSize: 11, color: TEXT_LIGHT, paddingHorizontal: 16, paddingBottom: 8, lineHeight: 16 },
  // Hidden tap area — no visible text, just invisible touchable
  adminHint:     { paddingVertical: 12 },
  adminHintText: { fontSize: 12, color: TEXT_LIGHT },
