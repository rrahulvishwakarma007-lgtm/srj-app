...
export default function GoldRatesScreen() {
  ...
  // PIN state — 'pin' = show PIN pad, 'form' = show rate form
  ...
  const openAdmin = () => {
    ...
    setAdminStep('pin'); // always start at PIN screen
    setAdminOpen(true);
  ...
  const closeAdmin = () => {
    Animated.timing(adminAnim, { toValue: 0, duration: 250, useNativeDriver: true }).start(() => {
      setAdminOpen(false);
      setAdminStep('pin'); // reset to PIN for next time
    });
  };

  // Called when PIN is verified correctly
  const onPinSuccess = () => {
    setAdminStep('form');
  };
...
const styles = StyleSheet.create({
  ...
  adminHint:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 12 },
  ...
  flash: {
    ...
    backgroundColor: PURPLE_DARK, paddingHorizontal: 20, paddingVertical: 12,
    borderRadius: 30, borderWidth: 1, borderColor: GOLD + '60',
