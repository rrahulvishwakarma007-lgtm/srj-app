...
function AppTabs({ user, onLoginPress, onLogout, openProduct, wishlist, selectedProduct, closeProduct, toggleWishlist, addToCart }: any) {
  ...
  return (
    <View style={{ flex:1, backgroundColor:Theme.bgPurpleDark }}>
        ...
        <Tab.Navigator
          screenOptions={({ route }) => ({
            ...
            tabBarIcon: ({ color, focused }) => {
              ...
              if (route.name === 'Reels') return (
                <View style={{ width:38,height:38,borderRadius:19,backgroundColor:focused?'#9B6ED4':'#EDE8F5',alignItems:'center',justifyContent:'center',marginTop:-6,elevation:focused?4:0 }}>
      ...
      <ProductModal
        visible={!!selectedProduct} product={selectedProduct}
        onClose={closeProduct} onAddToWishlist={toggleWishlist} onAddToCart={addToCart}
...
export default function App() {
  ...
  return (
    <SafeAreaProvider>
      ...
      {/* Intro screen — shown once on first open */}
      {showIntro ? (
        <IntroScreen onFinish={() => setShowIntro(false)}/>

      /* Login screen — optional, user can skip */
      ) : authView === 'login' ? (
        <LoginScreen
      ...
      /* Signup screen — optional, user can skip */
      ) : authView === 'signup' ? (
        <SignUpScreen
      ...
      /* Main app — always accessible, with or without login */
      ) : (
        <AppTabs
          user={user}
          onLoginPress={goToLogin}
